...
Exercise files are named `short-topic.<ext>` — no date prefix (dates are only for `log/`), and the name stays to 2-4 words so the folder listing is easy to scan. Extension matches the content: `.sql`/`.py` for actual code or queries, `.md` for writeup/reflection with no code.
...
4. **Math & Statistics** — the stats that actually show up in analysis and ML
5. **Visualization** — telling a straight story with a chart
